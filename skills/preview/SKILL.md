...
4. Delegate the live checks to `deploy`, including path/device screenshots, flows, and snapshots.
5. Publish markdown/json artifacts for the preview run.
6. Write a PR-comment-ready markdown summary with findings, flow results, deploy checks, and evidence paths.
7. Fail the verification job when readiness fails or deploy verification returns a critical status.
