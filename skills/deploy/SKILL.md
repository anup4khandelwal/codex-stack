...
2. Wait for the target to become ready before running browser checks.
3. Verify each configured path across the requested device presets.
4. Capture screenshots and collect HTTP plus console evidence for each page check.
5. Run any configured QA flows and snapshot comparisons.
6. Publish markdown/json/comment artifacts plus a screenshot manifest under the publish directory.
7. Mark the run as `critical`, `warning`, or `pass` based on HTTP failures, console drift, and QA findings.
